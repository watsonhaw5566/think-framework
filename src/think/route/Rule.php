...
abstract class Rule
{
     ...
     * 绑定模型.
     *
     * @param array|Closure|string $var       路由变量名 多个使用 & 分割
     * @param null|Closure|string  $model     绑定模型类
     ...
     * 指定路由中间件.
     *
     * @param array|Closure|string $middleware 中间件
     ...
     * 设置路由缓存.
     *
     * @param array|int|string $cache 缓存
     *
     * @return $this
     */
     ...
     */
    protected function dispatchMethod(Request $request, array|string $route, array $option = []): CallbackDispatch
    {
        if (is_string($route)) {
